...
export class RelayerSDKLoader {
  ...
  public load(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onerror = () => {
        console.log("[RelayerSDKLoader] script onerror from CDN, fallback to local... ");
        // Fallback to local UMD file in /public
        ...
        localScript.onload = () => {
          if (!isFhevmWindowType(window, this._trace)) {
            reject(
              new Error(
                `RelayerSDKLoader: Local Relayer SDK loaded from ${SDK_LOCAL_URL}, but window.relayerSDK is invalid.`
              )
            );
            return;
          }
        ...
        localScript.onerror = () => {
          reject(
            new Error(
              `RelayerSDKLoader: Failed to load Relayer SDK from CDN (${SDK_CDN_URL}) and local fallback (${SDK_LOCAL_URL}).`
            )
          );
        };
